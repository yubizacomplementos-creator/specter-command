...
export async function POST(request: NextRequest) {
  ...
  const channelSession = await prisma.botChannelSession.findUnique({
    where: {
  ...
  if (channelSession) {
    await prisma.botChannelSession.update({
  ...
  return NextResponse.redirect(redirectBackUrl(request, "/command/bot", { bot: "whatsapp_disconnected" }), 303);
}
